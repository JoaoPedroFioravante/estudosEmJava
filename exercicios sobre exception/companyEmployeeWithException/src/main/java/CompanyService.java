...
public class CompanyService {
    ...
    public void hire(String id, String name, String jobTitle, double salary, LocalDate dateOfEmployment){
        ...
        Objects.requireNonNull(employee, "não foi possivel adicionar o funcionario");
    ...
    public void payToTest(String id, LocalDate date){
        requireNonNullAndIsBlank(id, "id");
        if(date == null) throw new IllegalArgumentException("data não pode ser nula");
        Employee employee = repository.findById(id);
        if(employee == null) {
    ...
    public double averageSalary(LocalDate initial, LocalDate end){
        ...
        if(initial.isAfter(end)) throw new IllegalArgumentException("data inicial: "+initial+ " não pode ser depois da data final: "+end);
